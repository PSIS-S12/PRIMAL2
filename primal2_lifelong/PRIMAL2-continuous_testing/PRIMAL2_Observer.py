...
class PRIMAL2_Observer(ObservationBuilder):
    ...
    def _get(self, agent_id, all_astar_maps):
        ...
        for i in range(top_left[0], top_left[0] + self.observation_size):
            for j in range(top_left[1], top_left[1] + self.observation_size):
                ...
                # keep this map even if on goal, but guard against missing maps
                if self.world.agents[agent_id].distanceMap is not None:
                    pathlength_map[i - top_left[0], j - top_left[1]] = self.world.agents[agent_id].distanceMap[i, j]
                else:
                    pathlength_map[i - top_left[0], j - top_left[1]] = 0
            ...
            if cell_info[1] == 1:
                ...
                if corridor_id != current_corridor_id:
                    if len(self.world.corridors[corridor_id]['EndPoints']) == 1:
                        ...
                        deltax_map[position[0] - top_left[0], position[1] - top_left[1]] = (
                            self.world.corridors[corridor_id]['DeltaX'][(end_point_pos[0], end_point_pos[1])]
                        )
                        deltay_map[position[0] - top_left[0], position[1] - top_left[1]] = (
                            self.world.corridors[corridor_id]['DeltaY'][(end_point_pos[0], end_point_pos[1])]
                        )
                        blocking_map[position[0] - top_left[0], position[1] - top_left[1]] = self.get_blocking(
                            corridor_id,
                            0, agent_id,
    ...
    def get_astar_map(self):
        ...
        def get_single_astar_path(distance_map, start_position, path_len):
            ...
            if distance_map is None or start_position is None:
                return []
        ...
        for agentID in range(1, self.world.num_agents + 1):
            ...
            for i in range(self.num_future_steps - len(astar_path)):
                try:
                    ...
                    astar_path = [[self.world.agents[agentID].position]]
                    astar_path.extend([[self.world.agents[agentID].position]] * (self.num_future_steps - 1))
                    break
