...
class World:
    ...
    def put_goals(self, id_list, manual_pos=None):
        """
        Minimal goal assignment to make generator + env work.
        No lifelong logic, just assign goals safely.
        """

        # clear previous goals
        self.goals_map[self.goals_map > 0] = 0

        if manual_pos is not None:
            goals = manual_pos
        else:
            free_space = np.argwhere(self.state == 0)
            free_space = [tuple(pos) for pos in free_space]

            if len(free_space) < len(id_list):
                raise ValueError("Not enough free space for goals")

            chosen = np.random.choice(len(free_space), size=len(id_list), replace=False)
            goals = {agentID: free_space[i] for agentID, i in zip(id_list, chosen)}

        for agentID in id_list:
            x, y = goals[agentID]

            if self.state[x, y] == -1:
                raise ValueError(f"Invalid goal position for agent {agentID}")

            self.goals_map[x, y] = agentID
            self.agents[agentID].goal_pos = (x, y)
        ...
        if new_goals is not None:  # recursive breaker
            ...
            for agentID in id_list:
                ...
                if not self.isConventional:
                    ...
                    # 🔥 ALWAYS compute distance map (this is the fix)
